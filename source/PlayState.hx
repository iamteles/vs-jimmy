...
class PlayState extends MusicBeatState
{
	...
	var hardCodedOpponent:String = "jimmy";
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'coom-music' | 'welcome-back' | 'holding-on':
					curStage = 'jimmyroom';
					hardCodedOpponent = 'Jimmy';
				case 'tutorial':
					curStage = 'stage';
					hardCodedOpponent = 'gf-speaker';
				default:
					curStage = 'stage';
					hardCodedOpponent = SONG.player2;
			}
		}
		...
		switch (curStage)
		{
			...
			case 'jimmyroom': //Week 1
				/*
				bgB:BGSprite = new BGSprite('daRoom-broken', -1000, 200, 1, 1);
				...
				*/
				...
				bgBREAKING.antialiasing = ClientPrefs.globalAntialiasing;
		...
		dad = new Character(0, 0, hardCodedOpponent);
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						//health drain code stolen from vs selever :) link: https://github.com/DragShot/fnf-vs-selever-mod
	...
	override function stepHit()
	{
		...
		{
			case 'holding-on':
				if(curStep >= 388 && curStep < 1150) //388
				{
				...
				if(curStep >= 1155 && curStep < 9999) //1155
				{
					bgRoom.visible = false;
					bgB.visible = false;
					if(doMorelashes)
				...
				if(curStep >= 1440 && curStep < 9999) //1440
				{
					defaultCamZoom = 0.5;
	...
	override function beatHit()
	{
		...
		var gfPercent:Float;
		gfPercent = Math.floor(ratingPercent * 100);


		if (gfPercent > 89 && !Math.isNaN(ratingPercent))
		{
			gf.idleSuffix = "H";
		}
		else if (gfPercent < 70 && !Math.isNaN(ratingPercent))
		{
			gf.idleSuffix = "S";
		}
		else
